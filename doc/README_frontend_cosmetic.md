...
Note: These actions do not require recompiling the code. Feel free to contact us if you need help and advice.
...
All translation is performed in the /resources/data/translations.yaml. It contains a YAML dictionnary with all classes in use. For each entry, you can add your own translation, with a language prefix (e.g. "de", "it", "es") that is the same for each entry. Then, update the /resources/data/language.conf.
Please note that UI size has been set to fit French wording. Hence, it is suggested to keep a size of message shorter or equivalent to the French version length as much as possible.
...
To change the color of the bounding boxes (squares around the die patterns) or of the ruler (used to measure picture scale), modify /resources/data/language.conf file.
...
To add a new font, you can download it in .otf format (of course ensure that you have the correct license rights to do so), and paste the .otf in resources/styles/fonts/. You can then load them in a QSS file using the syntax:
@font-face {
